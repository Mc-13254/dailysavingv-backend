-- Patch (v3 - idempotent): safe to run regardless of how far the previous
-- (partially failed) attempt got. Each step checks the current state before
-- acting, so columns already renamed/added are simply skipped.
...
-- ---- 5) Sanity check: confirm the final column list ----
